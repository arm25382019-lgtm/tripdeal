...
type SearchPrice = {
  ...
  source?: string;
  source_role?: 'live_inventory' | 'price_discovery_only' | string;
  price_basis?: 'live_offer' | 'roundtrip_discovery' | 'one_way_discovery' | 'sum_of_exact_one_way_discovery';
};
...
function mergeReferences(exactRows: SearchPrice[], nearbyRows: ReferencePrice[]): ReferencePrice[] {
  const rows: ReferencePrice[] = [
    ...exactRows.map((row) => ({ ...row, reference_type: 'exact_date' as const, reference_day_distance: 0 })),
    ...nearbyRows,
  ];
...
export default function AirlineSearchPage() {
  ...
  const [livePrices, setLivePrices] = useState<SearchPrice[]>([]);
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      if (liveResult.status === 'fulfilled') {
        ...
        setLivePrices(liveResult.value.prices || []);
        if (liveResult.value.error) setLiveWarning(liveResult.value.error);
      } else {
        setLiveConfigured(true);
        setLivePrices([]);
        setLiveWarning(liveResult.reason instanceof Error ? liveResult.reason.message : 'Amadeus Live Search ใช้งานไม่ได้ชั่วคราว');
      }

      if (referenceResult.status === 'fulfilled') {
        setReferencePrices(mergeReferences(referenceResult.value.prices || [], referenceResult.value.reference_prices || []));
      } else {
        setReferencePrices([]);
      }

      if (liveResult.status === 'rejected' && referenceResult.status === 'rejected') {
        setError('ระบบค้นหาทั้ง Live และราคาอ้างอิงใช้งานไม่ได้ชั่วคราว');
      }
  ...
  const sorted = useMemo(() => [...livePrices].sort((a, b) => a.price - b.price), [livePrices]);
  ...
  const liveAirlines = useMemo(() => new Set(sorted.map((row) => row.airline)), [sorted]);
  const referenceAirlines = useMemo(() => new Set(references.map((row) => row.airline)), [references]);
  const referenceOnly = useMemo(() => references.filter((row) => !liveAirlines.has(row.airline)), [references, liveAirlines]);
  const missingFallbacks = useMemo(() => routeFallbacks.filter((fallback) => !liveAirlines.has(fallback.code) && !referenceAirlines.has(fallback.code)), [routeFallbacks, liveAirlines, referenceAirlines]);
  ...
  const openDeal = (row: SearchPrice, index: number) => {
    ...
    const q = new URLSearchParams({
      ...
      source: row.source_role === 'live_inventory' ? 'live' : 'reference',
  ...
  const renderReferenceCard = (reference: ReferencePrice, index: number) => {
    ...
    return <article className="air-result-card" key={`reference-${reference.airline}-${reference.departure_at}-${index}`}>
      <div className="air-result-main">
        <div className="air-result-tags"><span>{exactReference ? 'ราคาอ้างอิงตรงวัน' : `ราคาอ้างอิงใกล้วัน · ${dayDistance} วัน`}</span>{bookingUrl && <span>Airline Direct</span>}</div>
        <h3>{airlineDisplayName(reference.airline)}</h3>
        <p className="air-result-route">{reference.origin_airport || origin} → {reference.destination_airport || destination}</p>
        <p>รายการนี้ไม่ใช่ Live Inventory ของ TripDeal ใช้เพื่อช่วยค้นสายการบิน/ช่วงราคา แล้วให้เว็บไซต์สายการบินตรวจเที่ยวบินของวันที่เลือกอีกครั้ง</p>
        <p className="air-result-flight">ข้อมูลอ้างอิงวันที่ {dateTH(reference.departure_at)}{reference.flight_number ? ` · ${reference.airline}${reference.flight_number}` : ''}</p>
      </div>
      <div className="air-result-price">
        <strong>฿{money(reference.price)}</strong><span>ราคาอ้างอิง / คน</span>
        <small>{exactReference ? 'ข้อมูลราคาเคยพบตรงวัน · ไม่ใช่การล็อกที่นั่ง' : `ห่างจากวันที่เลือก ${dayDistance} วัน`}</small>
        {bookingUrl ? <button onClick={() => openOfficialSearch(reference.airline)}>ตรวจตรงกับ {airline?.name || airlineDisplayName(reference.airline)} <ChevronRight size={16}/></button> : <button disabled>ยังไม่มีลิงก์สายการบิน</button>}
      </div>
    </article>;
  };
  ...
  const renderFallbackCard = (fallback: ReturnType<typeof getRouteFallbackAirlines>[number], index: number) => {
    ...
    return <article className="air-result-card" key={`${origin}-${destination}-${fallback.code}-${index}`}>
      <div className="air-result-main">
        <div className="air-result-tags"><span>Low-cost / Airline Direct</span><span>ตรวจตรงสายการบิน</span></div>
        <h3>{airlineDisplayName(fallback.code)}</h3><p className="air-result-route">{origin} → {destination}</p>
        <p>{fallback.note || 'TripDeal มีข้อมูลว่าสายการบินนี้เกี่ยวข้องกับเส้นทาง แต่ให้เว็บไซต์ทางการยืนยันตารางบินและราคาของวันที่เลือก'}</p>
      </div>
      <div className="air-result-price"><strong>เช็กราคาจริง</strong><span>{airline?.name || airlineDisplayName(fallback.code)}</span><small>ตารางบิน ที่นั่ง และราคายืนยันกับสายการบิน</small>{bookingUrl ? <button onClick={() => openOfficialSearch(fallback.code, fallback.routeBookingUrl)}>เปิดหน้าจองสายการบิน <ChevronRight size={16}/></button> : <button disabled>ยังไม่มีลิงก์จอง</button>}</div>
    </article>;
  };
  ...
  return <div className="air-search-page">
    <header className="air-search-topbar"><button onClick={() => navigate('/')} aria-label="กลับหน้าแรก"><ArrowLeft size={21}/></button><Link to="/" className="air-search-brand"><Plane size={21} fill="currentColor"/>TripDeal</Link><span><ShieldCheck size={14}/> Hybrid Live</span></header>
    ...
    <main className="air-search-container">
      ...
      <div className="air-search-trust"><ShieldCheck size={17}/><div><strong>TripDeal Live Search = Amadeus Live + Airline Direct</strong><span>{liveConfigured ? `Full-service ใช้ Amadeus ${liveEnvironment === 'production' ? 'Production Live' : 'Test'} · Low-cost ใช้ Official airline URL/API เท่าที่รองรับ · Travelpayouts เป็นราคาอ้างอิงเท่านั้น` : `Amadeus Live ยังไม่ได้ตั้งค่า · ตอนนี้จะแสดง Airline Direct + ราคาอ้างอิงเท่านั้น · Directory รองรับ ${supportedAirlineCount}+ สายการบิน`}</span></div></div>
      {liveWarning && <div className="air-search-disclaimer"><strong>Live Search มีข้อขัดข้อง</strong><p>{liveWarning} · ระบบยังแสดงข้อมูลอ้างอิงและลิงก์สายการบินให้ใช้งานต่อได้</p></div>}

      {loading && <div className="air-search-empty"><Search size={24}/><strong>กำลังค้นหา Live Inventory + Airline Direct...</strong><span>Full-service ค้นจาก Amadeus ส่วนสาย Low-cost จะใช้ข้อมูลอ้างอิงและส่งไปตรวจที่สายการบินโดยตรง</span></div>}
      ...
      {!loading && !error && sorted.length > 0 && <>
        <div className="air-search-title"><div><h2>Live Offers · {sorted.length} ตัวเลือก</h2><p>ข้อมูลเที่ยวบินและราคาจาก Amadeus Live Search · ก่อนชำระเงินจริงให้สายการบินยืนยันอีกครั้ง</p></div><button onClick={() => navigate('/')}>ค้นหาใหม่</button></div>
        <div className="air-search-list">{sorted.map((row, index) => {
          ...
          return <article className={isBestValue ? 'air-result-card best' : 'air-result-card'} key={`live-${row.airline}-${row.flight_number}-${row.departure_at}-${row.price}-${index}`}>
            <div className="air-result-main"><div className="air-result-tags"><span className="best">● LIVE</span>{isBestValue && <span className="best">⭐ คุ้มที่สุด</span>}{isCheapest && !isBestValue && <span className="best">💸 ถูกที่สุด</span>}{airline && <span>จองตรงสายการบิน</span>}</div><h3>{airlineDisplayName(row.airline)}</h3><p className="air-result-route">{row.origin_airport || origin} → {row.destination_airport || destination}</p><p>{direct ? 'บินตรง' : `ขาไปต่อ ${row.transfers || 0} ครั้ง${trip === 'roundtrip' ? ` · ขากลับต่อ ${row.return_transfers || 0} ครั้ง` : ''}`}</p><p className="air-result-flight">ขาไป {row.flight_number ? `${row.airline}${row.flight_number}` : 'รอยืนยันเลขเที่ยวบิน'} · {dateTH(row.departure_at)}</p>{trip === 'roundtrip' && <p className="air-result-flight">ขากลับ {row.return_flight_number ? `${row.airline}${row.return_flight_number}` : 'รอยืนยันเลขเที่ยวบิน'} · {dateTH(row.return_at)}</p>}{Number(row.number_of_bookable_seats || 0) > 0 && <p className="air-result-flight">Amadeus รายงานที่นั่งที่จองได้อย่างน้อย {row.number_of_bookable_seats} ที่นั่งใน offer นี้</p>}<div className="air-score">Value Score <b>⭐ {score}/100</b><span><i style={{ width: `${score}%` }}/></span></div></div>
            <div className="air-result-price"><strong>฿{money(row.price)}</strong><span>{trip === 'roundtrip' ? 'ไป–กลับ' : 'เที่ยวเดียว'} / คน</span><small>Live offer · ราคาสุดท้ายยืนยันอีกครั้งก่อนจ่าย</small><button onClick={() => openDeal(row, index)}>ดูรายละเอียดและจองตรง <ChevronRight size={16}/></button></div>
          </article>;
        })}</div>
      </>}

      {!loading && !error && referenceOnly.length > 0 && <>
        <div className="air-search-title"><div><h2>Low-cost / สายการบินอื่นที่ควรตรวจเพิ่ม</h2><p>Travelpayouts ใช้เฉพาะราคาอ้างอิง ไม่เอามาปนกับ Live Inventory</p></div></div>
        <div className="air-search-list">{referenceOnly.slice(0, 12).map(renderReferenceCard)}</div>
      </>}

      {!loading && !error && missingFallbacks.length > 0 && <>
        <div className="air-search-title"><div><h2>Airline Direct เพิ่มเติม</h2><p>ไม่มี Live/ราคาอ้างอิงที่พอเชื่อถือได้ จึงให้ตรวจบนเว็บสายการบินโดยตรง</p></div></div>
        <div className="air-search-list">{missingFallbacks.map(renderFallbackCard)}</div>
      </>}

      {!loading && !error && !hasAnyResult && <div className="air-search-empty"><strong>ยังไม่พบ Live Offer หรือราคาอ้างอิงของเส้นทางนี้</strong><span>ถ้า Amadeus Live เปิดใช้งานแล้ว อาจเป็นเส้นทางที่อยู่นอก coverage หรือเป็น Low-cost carrier ที่ต้องตรวจตรงกับสายการบิน</span><button onClick={() => navigate('/')}>เปลี่ยนวันหรือค้นหาใหม่</button></div>}

      {!loading && !error && hasAnyResult && <div className="air-search-disclaimer"><strong>Hybrid Search ของ TripDeal</strong><p>ป้าย LIVE มาจาก Amadeus Flight Offers Search ส่วนรายการราคาอ้างอิงมาจาก Travelpayouts และจะไม่ถูกเรียกว่า Live Inventory สาย Low-cost ที่ไม่มี API จะพาไปตรวจและจองที่เว็บไซต์ทางการของสายการบินโดยตรง</p></div>}
    </main>
