...
export default function FlightSearchEngine() {
  ...
  return <div className="flight-search-engine">
    ...
    <div className="flight-search-route">
      <label className="flight-search-field">
        ...
        <select value={origin} onChange={(e) => setOrigin(e.target.value)}>
          <optgroup label="ประเทศไทย">
            {AIRPORTS.filter(a => a.region === 'Thailand').map(a => <option key={`o-${a.code}`} value={a.code}>{a.label}</option>)}
          </optgroup>
          <optgroup label="เอเชีย">
            {AIRPORTS.filter(a => a.region === 'Asia').map(a => <option key={`o-${a.code}`} value={a.code}>{a.label}</option>)}
          </optgroup>
        </select>
      </label>
      ...
      <label className="flight-search-field">
        ...
        <select value={destination} onChange={(e) => setDestination(e.target.value)}>
          <optgroup label="ประเทศไทย">
            {AIRPORTS.filter(a => a.region === 'Thailand').map(a => <option key={`d-${a.code}`} value={a.code}>{a.label}</option>)}
          </optgroup>
          <optgroup label="เอเชีย">
            {AIRPORTS.filter(a => a.region === 'Asia').map(a => <option key={`d-${a.code}`} value={a.code}>{a.label}</option>)}
          </optgroup>
        </select>
      </label>
    </div>
    ...
    <div className="flight-search-details">
      <label className="flight-search-field compact">
        <span><CalendarDays size={15}/> วันเดินทาง</span>
        <input type="date" min={toInputDate(today)} value={depart} onChange={(e) => { setDepart(e.target.value); if (returnDate < e.target.value) setReturnDate(e.target.value); }}/>
      </label>

      {tripType === 'roundtrip' && <label className="flight-search-field compact">
        <span><CalendarDays size={15}/> วันกลับ</span>
        <input type="date" min={depart || toInputDate(today)} value={returnDate} onChange={(e) => setReturnDate(e.target.value)}/>
      </label>}

      <div className="flight-search-field compact passenger-field">
        <span><Users size={15}/> ผู้โดยสาร</span>
        <div className="passenger-stepper">
          <button onClick={() => setAdults(Math.max(1, adults - 1))} aria-label="ลดจำนวนผู้โดยสาร"><Minus size={16}/></button>
          <strong>{adults} คน</strong>
          <button onClick={() => setAdults(Math.min(9, adults + 1))} aria-label="เพิ่มจำนวนผู้โดยสาร"><Plus size={16}/></button>
        </div>
      </div>
    </div>
